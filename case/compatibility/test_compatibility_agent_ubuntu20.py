...
vm_name = 'automation-compatibility-ubuntu20'
loop_num = 90
vtaps_name = 'NAME'
dst_ip = '114.114.114.114'


class TestCompatibilityAgentUbuntu20(CompatibilityBaseCase):
    ...
    def _setup_class(cls):
        ...
        with allure_step('step 1: creating ubuntu20 instance by Aliyun SDK'):
            cls.vm_private_ip = aliyun_sdk.create_instances_by_deploy(
                uuid=common_config.pytest_uuid, instance_names=[cls.vm_name],
                image_id=common_config.ali_image_ubuntu20_id_default
            )

    @classmethod
    def _teardown_class(cls):
        with allure_step('step 7: deleting ubuntu20 instance by Aliyun SDK'):
    ...
    @allure.feature('Agent compatibility ubuntu20')
    @allure.title('ubuntu20采集器deepflow-agent类型兼容性测试')
    @allure.description(
        'ubuntu20 system agent deepflow-agent deployment, checking logs, checking flows'
    )
    @pytest.mark.medium
    def test_compatibility_agent_ubuntu20(self):
        with allure_step(
            'step2: ubuntu20 system instance deploy the latest version of deepflow-agent'
        ):
            self.common_utils.vtaps_install_deepflow_agent_lastest_rpm(
                vtaps_mgt_ip=self.vm_private_ip[self.vm_name]
            )
        with allure_step('step3: checking ubuntu20 agent on DF vtaps list'):
        ...
        with allure_step('step4: checking ubuntu20 agent flow info'):
        ...
        with allure_step('step5: ubuntu20 agent abnormal restart scene'):
            utils.workload_v_force_kill_agent_action(
                vtaps_mgt_ip=self.vm_private_ip[self.vm_name]
            )
        with allure_step('step6: ubuntu20 agent check deepflow-agent logs'):
