...
vm_name = 'automation-compatibility-ubuntu16'
loop_num = 90
vtaps_name = 'NAME'
dst_ip = '114.114.114.114'


class TestCompatibilityAgentUbuntu16(CompatibilityBaseCase):
    ...
    def _setup_class(cls):
        ...
        with allure_step('step 1: creating ubuntu16 instance by Aliyun SDK'):
            cls.vm_private_ip = aliyun_sdk.create_instances_by_deploy(
                uuid=common_config.pytest_uuid, instance_names=[cls.vm_name],
                image_id=common_config.ali_image_ubuntu16_415_id
            )

    @classmethod
    def _teardown_class(cls):
        with allure_step('step 7: deleting ubuntu16 instance by Aliyun SDK'):
    ...
    @allure.feature('Agent compatibility ubuntu16')
    @allure.title('ubuntu16采集器deepflow-agent类型兼容性测试')
    @allure.description(
        'ubuntu16 system agent deepflow-agent deployment, checking logs, checking flows'
    )
    @pytest.mark.medium
    def test_compatibility_agent_ubuntu16(self):
        with allure_step(
            'step2: ubuntu16 system instance deploy the latest version of deepflow-agent'
        ):
            self.common_utils.vtaps_install_deepflow_agent_lastest_rpm(
                vtaps_mgt_ip=self.vm_private_ip[self.vm_name]
            )
        with allure_step('step3: checking ubuntu16 agent on DF vtaps list'):
        ...
        with allure_step('step4: checking ubuntu16 agent flow info'):
        ...
        with allure_step('step5: ubuntu16 agent abnormal restart scene'):
            utils.workload_v_force_kill_agent_action(
                vtaps_mgt_ip=self.vm_private_ip[self.vm_name]
            )
        with allure_step('step6: ubuntu16 agent check deepflow-agent logs'):
